...
(defn plot-one-res-square-aux [[[x1 y1] [x2 y2] :as line]
  ...
  (ineq.misc/log-val "l1:" ["x1" x1 "y1" y1 ", x2" x2 "y2" y2])
  (ineq.misc/log-val "l2:" ["rx1" rx1 "ry1" ry1 ", rx2" rx2 "ry2" ry2])
...
(defn normalize [kx ky c]
  (if (< (ineq.misc/abs kx) (ineq.misc/abs ky))
...
(defn calc-one-step [n pic base]
  (let [line (mk-line)
        ...
        init-w [0 0 0]
        [res-iters [wr0 wr1 wr2 :as res-w] :as ineq-res] (ineq init-w ys points)
        _ (ineq.misc/log-val "ineq res" ineq-res)
        ...
        _ (ineq.misc/log-val "diff p" diff-p)
        ]
    [res-iters diff-p]
    )
  )
...
(defn calc [n cnt pic]
  (let [base (int (rand 1000))
        _ (ineq.misc/log-val "n" n "cnt" cnt "base" base)
        res (for
                [_ (range cnt)]
              (calc-one-step n pic base))
        _ (ineq.misc/log-val "all step res" res)
